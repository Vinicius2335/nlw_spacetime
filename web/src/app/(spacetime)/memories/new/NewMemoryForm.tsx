...
export function NewMemoryForm() {
  ...
  async function handleCreateMemory(event: FormEvent<HTMLFormElement>) {
    ...
    if (fileToUpload) {
      ...
      const uploadResponse = await api
        .post<UploadResponse>('/upload', uploadFormData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        })
        .then((resp) => resp.data)

      coverUrl = uploadResponse.downloadUri
    }
  ...
  return (
    <form onSubmit={handleCreateMemory} className="flex flex-1 flex-col gap-2">
      ...
      <button
        type="submit"
        className="inline-block self-end rounded-full bg-green-500 px-5 py-3 font-alt text-sm uppercase leading-none text-black transition-colors hover:bg-green-700"
      >
        Salvar
      </button>
    </form>
